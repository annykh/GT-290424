...
-- min(поле) - вычисляет наименьшее значение
-- max(поле) - вычисляет наибольшее значение
-- sum(поле) - вычисляет сумму значений
-- count(*/поле) - вычисляет кол/во строк в запросе/в поле
-- avg(поле) - вычисляет среднее значение
...
-- Вывести сумму всех зарплат в компании
select sum(salary)
from employees;

-- Вывести среднее значение зарплаты в компании
select avg(salary)
from employees;
...
-- Вывести максимальную зарплату среди сотрудников у которых commission_pct пустой(null).
select max(salary)
from employees
where commission_pct is null;

-- Найти сумму зарплат, которые больше 10000.
select sum(salary)
from employees
where salary > 10000;
...
-- Найти среднее значение зарплаты сотрудников, которые платят комисси(commission_pct не пустой).
select avg(salary)
from employees
where commission_pct is not null;

-- Вывести кол/во сотрудников из департмента IT.
select count(*)
from employees emp
...
-- from table1
-- where column1 = (select aggregate(column) from employees);

-- Вывести имя и фамилию сотрудников с максимальнонй зарплатой 
-- 1. Найти максимальную зарплату
select max(salary)
from employees;

-- 2. Вывести имя и фамилию сотрудников с максимальнонй зарплатой 
...
-- 2. Найти сотрудников, у которых зарплата меньше средней зарплаты по компании
select first_name, last_name
from employees
...
where jb.job_title = 'Programmer' and salary > (select avg(salary) from employees);
